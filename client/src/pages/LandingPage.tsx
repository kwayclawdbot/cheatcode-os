...
// Design: Light theme, Robinhood/Spotify energy. Bold editorial typography.
// Asymmetric layout, strong color blocks, editorial sections.
// Target: convert cold visitors who don't know what CheatCode OS is.
//
// Sections:
// 1. Hero — bold headline, sub, dual CTA, animated ticker strip
// 2. Social proof strip — member count, creator count, signals/day
// 3. "What is CheatCode OS" — 3-column feature cards
// 4. Swipe Feed preview — mock swipe card demo
// 5. Intelligence demo — live ticker search teaser
// 6. Creator roster — all 13 creators with avatars
// 7. Trader identity / XP system teaser
// 8. Final CTA — join free
...
function TickerMarquee({ tickers }: { tickers: { symbol: string; score: number; direction: string }[] }) {
  if (!tickers.length) return null;
  const items = [...tickers, ...tickers]; // duplicate for seamless loop
  return (
    <div className="overflow-hidden border-y border-[#EAECF0] bg-white py-2.5">
      <motion.div
          ...
          return (
            <Link key={i} href={`/intelligence?ticker=${t.symbol}`}>
              <span className="inline-flex items-center gap-1.5 cursor-pointer hover:opacity-70 transition-opacity">
                <span className="ticker-mono text-sm font-bold text-[#101828]">{t.symbol}</span>
...
function FeatureCard({ icon, title, desc, accent }: { icon: React.ReactNode; title: string; desc: string; accent: string }) {
  return (
    <div className="group relative bg-white rounded-2xl border border-[#EAECF0] p-6 hover:shadow-lg transition-all duration-300 hover:-translate-y-1 overflow-hidden">
      ...
      <h3 className="font-bold text-[#101828] mb-2 text-base" style={{ fontFamily: "var(--font-display)" }}>{title}</h3>
...
function MockSwipeCard({ offset = 0, rotate = 0, zIndex = 0 }: { offset?: number; rotate?: number; zIndex?: number }) {
  return (
    <div className="absolute w-72 rounded-2xl overflow-hidden shadow-xl border border-[#EAECF0]"
      ...
      <div className="bg-white px-5 py-3 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-[#667085]">Swipe left to pass</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-xs text-[#667085]">Swipe right to like</span>
  ...
  return (
    <Link href={`/creators/${creator.slug || creator.id}`}>
      <div className="flex flex-col items-center gap-2 group cursor-pointer">
        <div className="relative w-14 h-14 rounded-full overflow-hidden border-2 border-[#EAECF0] group-hover:border-[#4DC820] transition-colors shadow-sm">
          {avatar ? (
        ...
        <p className="text-xs font-semibold text-[#344054] text-center leading-tight max-w-[72px] group-hover:text-[#4DC820] transition-colors">
...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] font-sans">
      ...
      {/* ── Minimal Landing Nav ── */}
      <nav className="sticky top-0 z-50 bg-white border-b border-[#EAECF0]">
          ...
          <Link href="/">
            <div className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="font-bold text-[#101828] text-base tracking-tight" style={{ fontFamily: "var(--font-display)" }}>
                ...
                <span className="text-xs font-bold text-[#667085] ml-1 tracking-widest">OS</span>
              </span>
            </div>
          </Link>
          <div className="flex items-center gap-3">
            <Link href="/home">
              <button className="text-sm font-semibold text-[#344054] hover:text-[#101828] transition-colors px-3 py-1.5">
                Sign In
              </button>
            </Link>
      ...
      <section className="relative overflow-hidden bg-white">
        ...
        <div className="max-w-6xl mx-auto px-6 pt-20 pb-16 relative">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#F0FDE8] border border-[#B6F08A] mb-6">
              <div className="w-1.5 h-1.5 rounded-full bg-[#4DC820] animate-pulse" />
              <span className="text-[11px] font-bold tracking-wide text-[#2E7A10]">THE SOCIAL PLATFORM FOR TRADERS</span>
            </div>
            <h1 className="text-5xl sm:text-6xl font-black text-[#101828] leading-[1.05] mb-5"
            ...
            <p className="text-lg text-[#344054] leading-relaxed mb-8 max-w-xl">
            ...
            <div className="flex flex-wrap gap-3 mb-10">
              ...
              <Link href="/intelligence">
                <button className="inline-flex items-center gap-2 text-base font-semibold px-6 py-3.5 rounded-xl bg-white border border-[#EAECF0] text-[#344054] hover:border-[#4DC820] hover:text-[#101828] transition-all">
            ...
            <div className="flex flex-wrap gap-6">
              {[
                ...
                <div key={s.label}>
                  <p className="text-2xl font-black text-[#101828]" style={{ fontFamily: "var(--font-display)" }}>{s.value}</p>
                  <p className="text-xs font-semibold text-[#667085]">{s.label}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ── Ticker Marquee ── */}
      <TickerMarquee tickers={radarTickers} />

      {/* ── What is CheatCode OS ── */}
      <section className="max-w-6xl mx-auto px-6 py-16">
        <div className="text-center mb-10">
          <p className="text-xs font-bold uppercase tracking-widest text-[#667085] mb-2">THE PLATFORM</p>
          <h2 className="text-3xl font-black text-[#101828]" style={{ fontFamily: "var(--font-display)", letterSpacing: "-0.01em" }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <FeatureCard
            icon={<Play size={18} />}
            title="Swipe Feed"
            desc="A TikTok-style horizontal swipe feed for short-form trade ideas and video clips. Swipe right to like and give the creator XP. Swipe left to pass."
            accent="#E8193C"
          />
          <FeatureCard
            icon={<BarChart2 size={18} />}
            title="Kai Intelligence"
            desc="AI that cross-references news, options flow, macro, insider activity, earnings, and curated content into one convergence score for any ticker."
            accent="#00AEEF"
          />
          <FeatureCard
            icon={<BookOpen size={18} />}
            title="Curated Education"
            desc="Handpicked content from 13 of the best trading and investing educators on YouTube — organized by topic, style, and experience level."
            accent="#7B2FBE"
          />
          <FeatureCard
            icon={<Users size={18} />}
            title="Trader Community"
            desc="Post trade ideas, share P&L, discuss setups, and follow traders whose style matches yours. Trader badges make it easy to find your people."
            accent="#4DC820"
          />
          <FeatureCard
            icon={<Star size={18} />}
            title="XP & Levels"
            desc="Earn XP for posting, sharing, and being right. Level up from Rookie to Legend. Your track record is your reputation."
            accent="#F79009"
          />
          <FeatureCard
            icon={<BookOpen size={18} />}
            title="Coaches Corner"
            desc="Book 1-on-1 sessions and buy courses from verified trader coaches. Coaches earn 80% of revenue and get their own dashboard."
            accent="#E8193C"
          />
        </div>
      </section>
      ...
      {/* ── Swipe Feed Preview ── */}
      <section className="bg-[#101828] py-16 overflow-hidden">
      ...
      <section className="max-w-6xl mx-auto px-6 py-16">
        <div className="text-center mb-10">
          <p className="text-xs font-bold uppercase tracking-widest text-[#667085] mb-2">THE EDUCATORS</p>
          <h2 className="text-3xl font-black text-[#101828] mb-3" style={{ fontFamily: "var(--font-display)", letterSpacing: "-0.01em" }}>
          ...
          <div className="flex flex-wrap gap-6 justify-center">
            {[
              ...
              <div key={name} className="flex flex-col items-center gap-2">
                <div className="w-14 h-14 rounded-full bg-[#F2F4F7] border border-[#EAECF0] flex items-center justify-center">
                  <span className="text-sm font-bold text-[#667085]">
                    {name.split(" ").map(w => w[0]).join("").slice(0, 2)}
                  </span>
                </div>
                <p className="text-xs font-semibold text-[#344054] text-center max-w-[72px] leading-tight">
        ...
        <div className="text-center mt-8">
          <Link href="/topics">
            <button className="inline-flex items-center gap-1.5 text-sm font-semibold text-[#344054] hover:text-[#101828] transition-colors">
      ...
      {/* ── Trader Identity ── */}
      <section className="bg-white border-y border-[#EAECF0] py-16">
        <div className="max-w-6xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-[#667085] mb-3">YOUR TRADER IDENTITY</p>
              <h2 className="text-3xl font-black text-[#101828] mb-4" style={{ fontFamily: "var(--font-display)", letterSpacing: "-0.01em" }}>
                Your style. Your level. Your reputation.
              </h2>
              <p className="text-[#344054] leading-relaxed mb-6">
              ...
              <div className="flex items-center gap-4">
                {[
                  ...
                  <div key={l.level} className="text-center">
                    ...
                    <p className="text-[10px] font-bold text-[#344054]">{l.level}</p>
                    <p className="text-[9px] text-[#667085]">{l.xp}</p>
                  </div>
                ))}
              </div>
            </div>
            <div className="bg-[#F9FAFB] rounded-2xl border border-[#EAECF0] p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="font-bold text-[#101828] text-sm">@jdtrader</p>
                ...
                <div className="ml-auto text-right">
                  ...
                  <p className="text-[10px] text-[#667085]">12,450 XP</p>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-3 mb-4">
                {[
                  ...
                  <div key={s.label} className="bg-white rounded-xl p-3 text-center border border-[#EAECF0]">
                    <p className="text-base font-black text-[#101828]">{s.value}</p>
                    <p className="text-[10px] text-[#667085]">{s.label}</p>
                  </div>
                ))}
              </div>
              <div className="space-y-2">
                <p className="text-[11px] font-bold uppercase tracking-wider text-[#667085]">Recent Ideas</p>
                {[
                  { ticker: "PLTR", dir: "↑", sentiment: "Bullish", xp: "+45 XP" },
                  { ticker: "NVDA", dir: "↓", sentiment: "Bearish", xp: "+32 XP" },
                ].map(idea => (
                  <div key={idea.ticker} className="flex items-center justify-between bg-white rounded-xl px-3 py-2.5 border border-[#EAECF0]">
                    <div className="flex items-center gap-2">
                      <span className="ticker-mono text-sm font-bold text-[#101828]">{idea.ticker}</span>
      ...
      <section className="max-w-6xl mx-auto px-6 py-20 text-center">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[#F0FDE8] border border-[#B6F08A] mb-6">
          <Flame size={12} className="text-[#2E7A10]" />
          <span className="text-[11px] font-bold tracking-wide text-[#2E7A10]">JOIN THE COMMUNITY</span>
        </div>
        <h2 className="text-4xl font-black text-[#101828] mb-4" style={{ fontFamily: "var(--font-display)", letterSpacing: "-0.02em" }}>
          Ready to trade smarter?
        </h2>
        <p className="text-base text-[#344054] mb-8 max-w-md mx-auto">
        ...
        <div className="flex flex-wrap gap-3 justify-center">
          ...
          <Link href="/home">
            <button className="inline-flex items-center gap-2 text-base font-semibold px-8 py-4 rounded-xl bg-white border border-[#EAECF0] text-[#344054] hover:border-[#4DC820] transition-all">
      ...
      {/* ── Footer ── */}
      <footer className="border-t border-[#EAECF0] bg-white py-8">
        <div className="max-w-6xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-sm text-[#667085]">© 2026 CheatCode OS. For educational purposes only. Not financial advice.</p>
          <div className="flex gap-6">
            {[
              ...
              <Link key={l.href} href={l.href}>
                <span className="text-sm text-[#667085] hover:text-[#101828] transition-colors cursor-pointer">{l.label}</span>
              </Link>
            ))}
          </div>
        </div>
