...
// Design: Twitter/X-meets-Robinhood. Light theme. Left sidebar (trending + radar),
// center feed (posts with trader badges), right sidebar (who to follow + leaderboard).
...
import {
  ...
  Bookmark, Share2, Plus, X, Image, BarChart2, Flame, Search,
  Filter, ChevronRight, Zap, Award, Users, Clock, Hash
} from "lucide-react";
...
// ─── Seed post data (illustrative structure — real posts come from backend) ───
...
const SEED_POSTS: Post[] = [
  {
    ...
    reactions: [
      { emoji: "🔥", label: "Bullish", count: 47, active: false },
      { emoji: "🐻", label: "Bearish", count: 8, active: false },
      { emoji: "👀", label: "Watching", count: 23, active: false },
    ],
    comments: 14,
    reposts: 6,
  },
  {
    ...
    reactions: [
      { emoji: "🔥", label: "Bullish", count: 89, active: false },
      { emoji: "❤️", label: "Like", count: 134, active: false },
    ],
    comments: 28,
    reposts: 19,
  },
  {
    ...
    reactions: [
      { emoji: "🐻", label: "Bearish", count: 62, active: false },
      { emoji: "🔥", label: "Bullish", count: 31, active: false },
      { emoji: "👀", label: "Watching", count: 44, active: false },
    ],
    comments: 41,
    reposts: 27,
  },
  {
    ...
    reactions: [
      { emoji: "🔥", label: "Bullish", count: 38, active: false },
      { emoji: "🐻", label: "Bearish", count: 52, active: false },
      { emoji: "👀", label: "Watching", count: 67, active: false },
    ],
    comments: 33,
    reposts: 8,
  },
  {
    ...
    reactions: [
      { emoji: "❤️", label: "Like", count: 24, active: false },
    ],
    comments: 18,
    reposts: 2,
  },
...
const SENTIMENT_CONFIG = {
  bullish: { color: "#4DC820", bg: "#F0FDE8", icon: TrendingUp, label: "Bullish" },
  bearish: { color: "#E8193C", bg: "#FFF0F3", icon: TrendingDown, label: "Bearish" },
  neutral: { color: "#F79009", bg: "#FFFBEB", icon: Minus, label: "Neutral" },
};

const POST_TYPE_CONFIG: Record<PostType, { label: string; color: string; bg: string }> = {
  trade_idea: { label: "Trade Idea", color: "#00AEEF", bg: "#EFF8FF" },
  pl_share: { label: "P&L Share", color: "#4DC820", bg: "#F0FDE8" },
  market_take: { label: "Market Take", color: "#7B2FBE", bg: "#F3E8FF" },
  chart_post: { label: "Chart", color: "#F79009", bg: "#FFFBEB" },
...
function PostCard({ post, onReact }: { post: Post; onReact: (postId: string, emoji: string) => void }) {
  ...
  const isLong = (post.text || post.thesis || "").length > 200;
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-2xl border border-[#EAECF0] p-4 hover:shadow-sm transition-shadow"
    >
      {/* Header */}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <Link href={`/traders/${post.user.handle.replace("@", "")}`}>
              <span className="font-bold text-[#101828] text-sm hover:underline cursor-pointer">{post.user.name}</span>
            </Link>
            <span className="text-xs text-[#667085]">{post.user.handle}</span>
          ...
          <div className="flex items-center gap-2 mt-0.5">
            ...
            <span className="text-[10px] text-[#98A2B3] ml-auto flex-shrink-0">{post.timestamp}</span>
          </div>
        </div>
      </div>
      ...
      {post.type === "trade_idea" && post.ticker && (
        <div className="flex items-center gap-3 mb-3 p-3 rounded-xl"
             style={{ background: sentConfig?.bg || "#F9FAFB" }}>
          <div className="flex items-center gap-2">
            <span className="ticker-mono text-base font-black text-[#101828]">{post.ticker}</span>
            {sentConfig && (
          ...
          {post.timeframe && (
            <span className="text-xs text-[#667085] ml-auto">{post.timeframe}</span>
          )}
        </div>
      )}
      ...
      {post.type === "pl_share" && post.pnl && (
        <div className="flex items-center gap-2 mb-3 p-3 rounded-xl"
             style={{ background: post.outcome === "win" ? "#F0FDE8" : "#FFF0F3" }}>
          <span className="text-lg font-black"
                style={{ color: post.outcome === "win" ? "#4DC820" : "#E8193C" }}>
            {post.pnl}
          </span>
          {post.ticker && (
            <span className="ticker-mono text-sm font-bold text-[#344054]">on {post.ticker}</span>
          )}
          <span className="ml-auto text-xs font-bold px-2 py-0.5 rounded-full"
                style={{
                  background: post.outcome === "win" ? "#4DC820" : "#E8193C",
                  color: "white"
                }}>
      ...
      {post.type === "trade_idea" && (post.entry || post.target || post.stop) && (
        <div className="grid grid-cols-3 gap-2 mb-3">
          {[
            ...
            <div key={k.label} className="bg-[#F9FAFB] rounded-xl p-2 text-center border border-[#EAECF0]">
              <p className="text-[9px] font-bold uppercase tracking-wide mb-0.5" style={{ color: k.color }}>{k.label}</p>
              <p className="ticker-mono text-xs font-bold text-[#101828]">{k.value}</p>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="flex items-center gap-1 flex-wrap mb-2">
        {post.reactions.map(r => (
          <button
            ...
            style={{
              background: r.active ? "#F0FDE8" : "#F9FAFB",
              borderColor: r.active ? "#4DC820" : "#EAECF0",
              color: r.active ? "#4DC820" : "#667085",
            }}
          >
            <span>{r.emoji}</span>
            <span>{r.count}</span>
      ...
      {/* Actions */}
      <div className="flex items-center gap-4 pt-2 border-t border-[#F2F4F7]">
        <button className="flex items-center gap-1.5 text-xs text-[#667085] hover:text-[#101828] transition-colors">
          <MessageCircle size={13} /> <span>{post.comments}</span>
        </button>
        <button className="flex items-center gap-1.5 text-xs text-[#667085] hover:text-[#4DC820] transition-colors">
          <Repeat2 size={13} /> <span>{post.reposts}</span>
        </button>
        <button className="flex items-center gap-1.5 text-xs text-[#667085] hover:text-[#101828] transition-colors">
          <Bookmark size={13} />
        </button>
        <button className="flex items-center gap-1.5 text-xs text-[#667085] hover:text-[#101828] transition-colors ml-auto">
          <Share2 size={13} />
        </button>
      </div>
    </motion.div>
...
function ComposeBox({ onPost }: { onPost: (text: string) => void }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#EAECF0] p-4 mb-4">
      <div className="flex gap-3">
        ...
        <div className="flex-1">
          <textarea
            ...
            rows={3}
            className="w-full text-sm text-[#101828] placeholder:text-[#98A2B3] resize-none focus:outline-none bg-transparent leading-relaxed"
          />
          <div className="flex items-center gap-2 mt-2 pt-2 border-t border-[#F2F4F7]">
            <div className="flex gap-1">
              {(["trade_idea", "pl_share", "market_take", "question"] as PostType[]).map(t => (
                <button
                  ...
                  style={{
                    background: type === t ? POST_TYPE_CONFIG[t].bg : "#F9FAFB",
                    color: type === t ? POST_TYPE_CONFIG[t].color : "#98A2B3",
                    border: `1px solid ${type === t ? POST_TYPE_CONFIG[t].color + "40" : "#EAECF0"}`,
                  }}
                >
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-[#EAECF0] p-4">
        <h3 className="font-bold text-[#101828] text-sm mb-3 flex items-center gap-2">
          <Flame size={14} className="text-[#E8193C]" /> Trending Now
        </h3>
        <div className="space-y-2">
          {trending.map((t, i) => {
            const dir = SENTIMENT_CONFIG[t.dir as Sentiment];
            return (
              <Link key={t.tag} href={`/intelligence?ticker=${t.tag}`}>
                <div className="flex items-center justify-between py-1.5 hover:bg-[#F9FAFB] rounded-xl px-2 -mx-2 transition-colors cursor-pointer">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-[#98A2B3]">#{i + 1}</span>
                    <span className="ticker-mono text-sm font-bold text-[#101828]">{t.tag}</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className="text-[10px] text-[#667085]">{t.posts} posts</span>
                    <span className="text-xs font-bold" style={{ color: dir.color }}>
      ...
      {radarTickers.length > 0 && (
        <div className="bg-white rounded-2xl border border-[#EAECF0] p-4">
              ...
              return (
                <Link key={t.symbol} href={`/intelligence?ticker=${t.symbol}`}>
                  <div className="flex items-center justify-between py-1.5 hover:bg-[#F9FAFB] rounded-xl px-2 -mx-2 transition-colors cursor-pointer">
                    ...
                    <div className="flex items-center gap-1.5">
                      <div className="h-1.5 w-12 bg-[#F2F4F7] rounded-full overflow-hidden">
          ...
          <Link href="/intelligence">
            <button className="w-full mt-3 text-xs font-bold text-[#667085] hover:text-[#101828] transition-colors flex items-center justify-center gap-1">
          ...
          {[
            ...
            <div key={u.handle} className="flex items-center gap-2.5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-[#667085]">{u.style} · {u.level}</p>
              </div>
              <button
                onClick={() => toast.success(`Following ${u.name}!`, { duration: 1500 })}
                className="text-[10px] font-bold px-2.5 py-1 rounded-lg border border-[#4DC820] text-[#4DC820] hover:bg-[#F0FDE8] transition-colors flex-shrink-0"
              >
                Follow
              </button>
            </div>
...
export default function CommunityPage() {
  ...
  const handlePost = (text: string) => {
    const newPost: Post = {
      ...
      text,
      reactions: [
        { emoji: "🔥", label: "Bullish", count: 0, active: false },
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB]">
      ...
      {/* ── Page Header ── */}
      <div className="bg-white border-b border-[#EAECF0] sticky top-14 z-30">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between py-3">
            <h1 className="font-black text-[#101828] text-lg" style={{ fontFamily: "var(--font-display)" }}>
              Community
            </h1>
            <div className="flex items-center gap-2">
              <div className="relative hidden sm:block">
                ...
                <input
                  ...
                  className="pl-8 pr-3 py-2 text-xs bg-[#F9FAFB] border border-[#EAECF0] rounded-xl focus:outline-none focus:border-[#4DC820] transition-colors w-48"
                />
              </div>
              <Link href="/feed">
            ...
            <div className="space-y-3">
              ...
              {filteredPosts.length === 0 && (
                <div className="text-center py-12">
                  <p className="text-[#667085] text-sm">No posts found. Be the first to post!</p>
                </div>
              )}
            </div>
          </div>
